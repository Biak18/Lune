...
export default function AdminProductsScreen() {
  ...
  const toggle = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      adminService.toggleProductActive(id, isActive),
    onSuccess: () => {
  ...
  const toggleCat = useMutation({
    mutationFn: ({ id, is_active }: { id: string; is_active: boolean }) =>
      adminService.updateCategory(id, { is_active }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin", "categories"] });
      qc.invalidateQueries({ queryKey: ["products"] });
    },
  ...
  const saveEditCat = useMutation({
    mutationFn: () => {
      ...
      return adminService.updateCategory(editingCat.id, {
      ...
      <SafeAreaView style={styles.root} edges={["top"]}>
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>
            Active toggles reflect instantly on Home/Shop via query invalidation
          </Text>
        </View>
        ...
        <View style={styles.catSection}>
          ...
          <View style={styles.catForm}>
            ...
            <Pressable
              ...
              style={[styles.addBtn, createCat.isPending && { opacity: 0.6 }]}
            >
          ...
          {createCat.isError ? (
            <Text style={styles.error}>
              {String((createCat.error as Error).message)}
            </Text>
          ) : null}
          <ScrollView
            horizontal
                ...
                {editingCat?.id === c.id ? (
                  ...
                  >
                    ...
                    <Pressable
                      ...
                      style={styles.miniBtn}
                    >
                      <Text style={styles.miniText}>Cancel</Text>
                    </Pressable>
                  </View>
                ) : (
                  <>
                    <Pressable
                      onPress={() => setEditingCat(c)}
                      style={{ flex: 1 }}
                    >
                    ...
                    <Switch
                      value={!!c.is_active}
                      onValueChange={(v) =>
                        toggleCat.mutate({ id: c.id, is_active: v })
                      }
