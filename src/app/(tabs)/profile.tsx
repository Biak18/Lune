  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false}>
        ...
        {/* Identity — open composition, serif name */}
        ...
        {/* Stats — editorial spec strip */}
        <Reveal index={1} style={styles.statsRow}>
          <Stat label="Orders" value={String(orders?.length ?? 0)} loading={ordersLoading} />
          <View style={styles.statDivider} />
          <Stat label="Wishlist" value={String(wishlistIds?.size ?? 0)} loading={wishlistLoading} />
          <View style={styles.statDivider} />
          <Stat label="Points" value={String(loyalty?.points ?? 0)} loading={loyaltyLoading} />
        </Reveal>

        {/* Boutique grouped actions — open list, no card chrome */}
        <Reveal index={2} style={styles.section}>
          <Text style={styles.sectionTitle}>Shopping</Text>
          <View>
            <MenuRow icon="bag-outline" label="My orders" sub="Track & returns" onPress={() => router.push("/orders" as any)} />
            <View style={styles.menuDivider} />
            <MenuRow icon="heart-outline" label="Wishlist" sub="Saved pieces" onPress={() => router.push("/(tabs)/wishlist" as any)} />
            <View style={styles.menuDivider} />
            <MenuRow icon="location-outline" label="Addresses" sub="Shipping details" onPress={() => router.push("/addresses" as any)} />
            {isAdmin ? (
              <>
                <View style={styles.menuDivider} />
        ...
        {/* Membership — the one feature card */}
            ...
            <View style={styles.activityList}>
              {tx.slice(0, 3).map((t) => (
                <View key={t.id} style={styles.activityRow}>
                  ...
                  {t.description ? <Text style={styles.activityPointsSub} numberOfLines={1}>{t.description}</Text> : null}
        ...
        <Reveal index={5} style={styles.footer}>
          <Pressable
...
const styles = StyleSheet.create({
  ...
  identity: { flexDirection: "row", gap: 14, alignItems: "center", paddingHorizontal: 4 },
  ...
  name: { fontFamily: fontFamily.display, fontSize: 24, lineHeight: 28, color: colors.foreground, letterSpacing: -0.3 },
  email: { fontSize: 12, color: colors.muted, lineHeight: 16 },
  memberSince: { fontSize: 11, color: colors.muted, marginTop: 1 },
  statsRow: {
    flexDirection: "row",
  ...
  statValue: { fontFamily: fontFamily.display, fontSize: 20, color: colors.foreground, letterSpacing: -0.2 },
  ...
  menuDivider: { height: StyleSheet.hairlineWidth, backgroundColor: colors.border, marginLeft: 30 },
  activityList: { gap: 8 },
  ...
  activityPointsSub: { flex: 1, fontSize: 11, color: colors.muted },
